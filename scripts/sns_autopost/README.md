...
- **返信下書き（2026-07-17復活・docs/58 FR-07）**: 上位 `LEAD_REPLY_MAX`(既定3)件には
  ✍️付きで「そのままコピペで送れる返信下書き」が添えられる。長押しコピー→リンク先で
  貼って送るだけ（送信は必ず自分の指。返信を送るAPIコールは存在しない）。
  下書きは検証ゲート（URL/タグ/誘導語/医療断定/歌手名/未知カタカナ語の棄却）を通過した
  文面のみ。Geminiキー無しなら監修済みテンプレで動く。`LEAD_REPLY_DRAFTS=0` で無効化。
- テスト: `python tests/qa_leads.py` / `python tests/qa_reply_drafts.py`（いずれも外部API/キー不要）。
