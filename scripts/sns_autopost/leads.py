...
# 2026-07-06 追加絞り込み: 「音痴」クエリはさらに、アニメ等のバズった話題（例:
# キャラクターが音痴という設定を面白がる投稿）に便乗した第三者コメントを大量に拾った
# （本人の悩みではない）。require を「自分の悩みとして直したい/苦手」という語に絞った。
# 併せて、運用者が明示的に狙う3テーマ（歌全般の上達願望/ミックスボイス/カラオケ上達）を
# 優先クエリ（PRIORITY_QUERY_IDS）として新設し、毎回の実行で必ず含める。
LEAD_QUERIES = [
    # --- 優先3テーマ（毎回必ず実行。運用者が明示的に指定した狙い） -----------------
    {"id": "sing_better", "query": "歌 上手くなりたい",     "intent": "歌が上手くなりたい",
     "keywords": ["上手くなりたい", "うまくなりたい", "上達したい", "歌が下手", "もっと上手く"]},
    {"id": "mixvoice_want", "query": "ミックスボイス 出したい", "intent": "ミックスボイスを出せるようになりたい",
     "keywords": ["ミックスボイス", "出したい", "出せるように", "出せない", "地声", "裏声"]},
    {"id": "karaoke_up",  "query": "カラオケ 上手く歌いたい", "intent": "カラオケでうまく歌いたい",
     "keywords": ["カラオケ", "上手く歌いたい", "うまく歌いたい", "歌いたい", "高得点"]},
    # --- 技術特化クエリ（優先3テーマを補完。個別の発声課題は上達願望の具体形として有効） ---
    ...
    {"id": "onchi",      "query": "歌 音痴",              "intent": "音程への自信のなさ",
     ...
     "require": ["直したい", "治したい", "克服したい", "なおしたい", "下手", "苦手"]},
...
# 優先3テーマ（運用者の明示指定。日替わりローテーションの対象外＝毎回必ず実行）。
PRIORITY_QUERY_IDS = ("sing_better", "mixvoice_want", "karaoke_up")
