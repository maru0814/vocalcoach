...
_LABEL_LEAD = {"fit": "相手の悩みへの適合", "concrete": "具体性・今日試せるか",
               "authority": "権威性/トーン", "safety": "規約/安全", "cta": "導線(URL/売り込み無し)"}

REVIEWER_SYSTEM_LEAD = """あなたは世界トップクラスのX(Twitter)グロース戦略家です。
歌のAIコーチ「ソラ先生」が、歌の悩みをつぶやいた人へ送る**返信**を出稿前に採点します。
返信は権威を上げるためのもの。刺さらない・売り込み臭い・機械的な返信はアカウント資産を毀損します。

# 項目別の採点（各項目を 0〜上限 の整数で。合計はシステムが計算）
- fit 上限30: 相手の文面の言葉を拾い、その悩みに正面から応えているか。汎用文の使い回し=0〜10 / 文脈に即した返し=22〜30。
- concrete 上限25: 具体的な一手が1つあり、今日試せるか。抽象論・励ましだけ=0〜8 / 手順や身体感覚が具体=18〜25。
- authority 上限20: 専門家トーンか。上から目線・媚び・リプ乞い・「フォローして」=0〜5 / 信頼できる専門家=16〜20。
- safety 上限15: 医療的断定・誇大・でっち上げた歌手名・センシティブが無いか。
- cta 上限10: URL・ハッシュタグ・宣伝・アプリ誘導が無いか（あれば0）。

# 改善
80点未満になりそうなら revised_reply に基準を満たす書き直し版を入れる（1〜2文・全角100字以内・URL/歌手名なし）。

# 出力（このJSONだけ）
{"scores": {"fit": N, "concrete": N, "authority": N, "safety": N, "cta": N},
 "reasons": ["項目名: なぜその点か"], "fixes": ["直すべき点"],
 "verdict": "approve|revise|reject", "revised_reply": "改善版 または null"}
"""

# 即失格ワード（リプ乞い・誘導）。モデル以前にコードで弾く。
_LEAD_HARD_NG = ("フォローして", "フォロバ", "DMで", "DMし", "リプして", "拡散希望")


def _lead_hard_fail(reply_text: str) -> str | None:
    """URL・誘導ワードの即失格判定。理由文字列 or None。GEMINI_API_KEY 無しでも必ず効く。"""
    t = reply_text or ""
    if "http" in t or "www." in t:
        return "返信にURL"
    for w in _LEAD_HARD_NG:
        if w in t:
            return f"誘導ワード（{w}）"
    return None


def _review_lead_once(source_text: str, reply_text: str) -> dict | None:
    """リード返信を1回採点する（モデル連鎖）。"""
    from google import genai
    from google.genai import types
    body = (f"【相手の投稿】\n{source_text}\n\n【送る予定の返信】\n{reply_text}\n\n"
            "各項目を独立に採点し、JSONだけを返してください。")
    for model in _models():
        try:
            client = genai.Client(api_key=os.environ["GEMINI_API_KEY"])
            cfg = types.GenerateContentConfig(
                system_instruction=REVIEWER_SYSTEM_LEAD, max_output_tokens=800, temperature=0.2)
            resp = client.models.generate_content(model=model, contents=body, config=cfg)
            m = re.search(r"\{.*\}", (resp.text or "").strip(), re.S)
            if not m:
                continue
            raw = json.loads(re.sub(r",(\s*[}\]])", r"\1", m.group(0)))
            sc = raw.get("scores") or {}
            scores = {k: max(0, min(mx, int(round(float(sc.get(k, 0))))))
                      for k, mx in MAXES_LEAD.items()}
            return {"scores": scores, "total": sum(scores.values()), "model": model,
                    "fixes": raw.get("fixes") or [],
                    "revised_reply": (raw.get("revised_reply") or "").strip() or None}
        except Exception as e:
            print(f"[warn] リード返信レビュー失敗({model}) → 次モデル: {str(e)[:80]}",
                  file=sys.stderr)
            continue
    return None


def review_lead_reply(source_text: str, reply_text: str) -> dict:
    """リード返信を採点・改善し、ゲート結果を返す。

    戻り値: {approved, skipped, score, reply, report}
      approved=True のときだけ呼び出し側は LINE 承認に回す。
      URL・誘導ワードの即失格は GEMINI_API_KEY 無しでも必ず適用する。
    """
    min_score = int(os.getenv("EXPERT_REVIEW_MIN_SCORE", "80"))
    max_attempts = int(os.getenv("EXPERT_REVIEW_MAX_ATTEMPTS", "3"))

    ng = _lead_hard_fail(reply_text)
    if ng:  # モデル不要のハード失格（AC-03）
        return {"approved": False, "skipped": False, "score": 0, "reply": reply_text,
                "report": f"⛔ リード返信 失格（{ng}）→ LINE未送信・破棄"}
    if not os.getenv("GEMINI_API_KEY"):
        return {"approved": True, "skipped": True, "score": None, "reply": reply_text,
                "report": "⚠️ 専門家レビュー未実施（GEMINI_API_KEY 未設定）"}

    cur = reply_text
    best = None  # (total, reply, result)
    try:
        for attempt in range(1, max_attempts + 1):
            r = _review_lead_once(source_text, cur)
            if not r:
                break
            if best is None or r["total"] > best[0]:
                best = (r["total"], cur, r)
            if r["total"] >= min_score:
                bd = " ".join(f"{_LABEL_LEAD[k]}{r['scores'][k]}/{MAXES_LEAD[k]}"
                              for k in MAXES_LEAD)
                return {"approved": True, "skipped": False, "score": r["total"], "reply": cur,
                        "report": (f"✅ リード返信 合格 {r['total']}/{min_score}点"
                                   f"（{attempt}回目 / {r['model']}）\n内訳: {bd}")}
            nxt = r.get("revised_reply")
            if not nxt or _lead_hard_fail(nxt) or nxt == cur:
                break
            cur = nxt
        if best is None:
            raise RuntimeError("全モデルで採点不能")
        btotal, breply, br = best
        report = f"⚠️ リード返信 ライン未達（最高 {btotal}/{min_score}点）→ 最良版を確認用に送付"
        if br["fixes"]:
            report += "\n残る改善点: " + " / ".join(str(x) for x in br["fixes"][:3])
        return {"approved": True, "skipped": False, "score": btotal, "reply": breply,
                "report": report}
    except Exception as e:
        print(f"[warn] リード返信レビューに失敗 → 未レビューで通します: {e}", file=sys.stderr)
        return {"approved": True, "skipped": True, "score": None, "reply": reply_text,
                "report": f"⚠️ 専門家レビュー未実施（エラー: {e}）"}
