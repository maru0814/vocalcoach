...
# アーティスト発声解説(pillar=artist_analysis)の追加基準 ★
実在アーティストの歌声を発声の仕組みから解説し、声タイプ診断へ誘導する型。次を厳しく見る:
- 実名: 本投稿(hook)にアーティスト実名が残っているか。実名が無い/型名だけ = hook・target を大幅減点。
- 推定の作法: 本人の発声を測定したかのような断定は誤情報。「〜に聞こえる」「音源からの推定」の
  作法を欠いた断定表現があれば safety 0〜2。作法を守った上で具体的なら concrete は満点まで可。
- リスペクト: 本人を貶す表現・他アーティストとの優劣比較・歌詞の引用が1つでもあれば verdict=reject
  （数字が取れても資産毀損。改善版でも必ず除去する）。
- 導線: リプ本体の末尾に「この声質は8タイプだと【◯◯】→あなたはどのタイプ？」の診断ブリッジが
  あれば algo 加点。無ければ revised_reply で足す。
