...
> - 2026-07-06 方針転換: 運用者フィードバックにより**返信文生成・返信用専門家ゲートを
...
> - 2026-07-06 追加絞り込み: 運用者が明示した3テーマ（歌全般の上達願望/
>   ミックスボイス/カラオケ上達）を`leads.PRIORITY_QUERY_IDS`として新設し、日替わり
>   ローテーションの対象外で毎回必ず実行するよう`lead_finder.source_search`を改修。
>   併せて本番実データで判明した「音痴」クエリのノイズ（バズった話題への第三者便乗
>   コメントの誤選別）を、本人の悩み語必須の`require`強化で是正した。
> - **2026-07-17 方針再転換（本改訂）**: 運用者指示（docs/62）により返信下書きを
>   `reply_drafts.py` として復活（docs/58 FR-07）。旧 `lead_reply.py` との違いは、
>   禁止事項をプロンプトで頼むだけでなく **`validate()` が出力側で強制**すること
>   （固有名詞ガード＝未知カタカナ語の棄却・`themes.ARTISTS_BY_ID` 実名禁止・
>   URL/タグ/誘導語/医療断定の棄却・棄却時は監修済みテンプレへフォールバック）。
>   ダイジェスト上位 `LEAD_REPLY_MAX`（既定3）件にのみ付与。送信は引き続き人間のみ。
...
| ❌ 削除 | ~~`lead_reply.py`~~ | 返信下書き生成（旧）。2026-07-06方針転換で廃止・ファイル削除 |
| 🆕 新規 | `reply_drafts.py` | 返信下書き生成（2026-07-17復活版）。Gemini任意＋監修テンプレ、`validate()` で固有名詞ガード等を出力側強制。writeコールなし |
...
```
- 候補ソースは安い順に ①`--input`（oEmbed・¥0）②メンション（owned read）③API検索（post read。優先3テーマは毎回必ず実行し、残り枠を他クエリで日替わりローテーション、`LEAD_QUERIES_PER_RUN` 本/回）。
...
- 環境変数（既存規約に追加）:
  ...
  | `LEAD_QUERIES_PER_RUN` | 4 | 1回の実行で回す検索クエリ数（優先3テーマ＋ローテ1が既定） |
...
- 実装済みハーネス: `tests/qa_leads.py`（42ケース。外部API/LINE/Gemini非依存で実行可）、
  `tests/qa_reply_drafts.py`（18ケース。FR-07/AC-03改/AC-09: 全テンプレの検証通過・
  歌手名/でっち上げカタカナ語の棄却・URL/タグ/誘導/医療断定の棄却・キー無しテンプレ
  フォールバック・同文連投検出・ダイジェスト行への付与・write API不在）。
...
**実装状況**: 実装済み（`leads.py`/`lead_finder.py`/`lead_metrics.py`、`approval_queue.py`/`line_client.py`/`webhook.py`/`expert_review.py`拡張・縮小、`lead_reply.py`削除、`tests/qa_leads.py` 42ケースPASS）。デプロイ済み・本番検証済み。
**2026-07-17追記**: `reply_drafts.py` 新設＋`lead_finder.py`/`line_client.py` 拡張（FR-07）。`tests/qa_reply_drafts.py` 18ケースPASS・`tests/qa_leads.py` 既存40ケースPASS（ローカル）。本番反映はデプロイ後に `--dry-run` で確認。
