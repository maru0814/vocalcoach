# 92. レポート — Gemini モデル移行検証（llm_audio_model / llm_analysis_model）

- 作成日: 2026-08-05（2026-08-06 に docs/91 の調査結果を統合）
- 対象: `backend/app/core/config.py` の `llm_audio_model` / `llm_analysis_model`（いずれも `gemini-2.5-flash` 固定）
- 関連: **docs/91（会話エラー修正と課題継続コーチング＝チャット経路側の同根調査）**, docs/43（ゼロベース個人最適FB）, docs/66（会話モードと対話モデル格上げ）, docs/42（FB品質基準・単一ソース）
- 実測環境: `backend/.venv`（google-genai 2.7.0）＋本番と同じ `backend/app/coaching/llm.py` の関数を直接呼び出し。
  §0 の本番検証のみ本番コンテナ内（`docker exec docker-backend-1`）で本番APIキーを使用

---

## 0. 総括 — docs/91 と本レポートの統合結論（2026-08-06）

チャット経路（docs/91・PR #279）と音声/分析経路（本レポート）は、**独立に調査して同一の根本原因に到達した**。

### 共通の根本原因
**`-latest` エイリアスが指す実体が Gemini 3 系へ切り替わり、`thinking_budget=0` が
`400 INVALID_ARGUMENT` で拒否されるようになった。** 両調査とも実測で確認している。

| | docs/91（チャット経路） | 本レポート（音声/分析経路） |
| --- | --- | --- |
| 症状 | テキスト会話が全滅（無言でルールベースに落ちる） | 未発生（既定が 2.5 固定のため潜在） |
| 対処 | `_thinking_off()`＝2.5 系以外には `thinking_config` を送らない | `_safe_thinking_budget()` ＋ `_safe_max_tokens()` |
| 検証方法 | ユニットテスト（モック） | **実APIコール** |

### 統合して分かった2点

**(1) `_thinking_off()` だけでは音声パスは守れない。**
thinking をモデル既定に委ねると思考トークンが `max_output_tokens` を食い、本文が途中で切れる。
実測: `gemini-flash-latest`・`thinking_config` 非送信・`max=600` で `finish=MAX_TOKENS`。
`analyze_pronunciation` は `llm_max_tokens=400` を使っていたため被害が大きい（§2-3）。
→ 音声パスは「最小予算を明示 ＋ 出力枠の下限を確保」の2段構えが要る。判定関数自体は
`_supports_thinking_budget()` に一本化した（真実の источник を1つに保つ）。

**(2) PR #279 の固定先 `gemini-2.5-flash-lite` はすでに 404 で死んでいた。**
docs/91 の検証は `_thinking_off()` の戻り値を確認するユニットテストのみで、
**固定先のモデルが実在するかを実APIで確かめていなかった。** その結果、
`400 INVALID_ARGUMENT` を `404 NOT_FOUND` に置き換えただけで、症状（無言のフォールバック）は
変わらないまま本番へ出た。

### 本番の実測（2026-08-06・本番コンテナ内・本番APIキー）

```
llm_model      = gemini-flash-lite-latest   ← VPSの.envが上書き。コード既定の固定が効いていない
llm_chat_model = gemini-2.5-flash-lite      ← 上書き無し＝コード既定がそのまま＝404 で死亡
```

| モデル | 結果 | 実際の会話形での応答（N=5・履歴4ターン・max=400） |
| --- | --- | --- |
| `gemini-2.5-flash-lite` | **404 NOT_FOUND** | — |
| `gemini-flash-lite-latest` | OK | 1.0s / STOP 5-5 / 思考tok 0 |
| `gemini-3.5-flash-lite` | OK | 1.1s / STOP 5-5 / 思考tok 0 |
| `gemini-3.1-flash-lite` | OK | 1.3s / STOP 5-5 / 思考tok 0 |
| `gemini-2.5-flash` | OK | 1.8s / STOP 5-5 / 思考tok 0 |

`llm_chat_model` は `generate_reply()`（チャット返答）と `generate_coach_comment()`（録音FBの
コーチコメント）が使う。**したがって #279 デプロイ後もチャットは復旧していない。**

### 是正
`llm_model` / `llm_chat_model` を **`gemini-3.5-flash-lite`** に固定する。
現行実効値（エイリアス）との差は **+0.1秒**で体感差なし。docs/91 自身が
「エイリアスは 3.5 系に切り替わった」と結論しており、3.5-flash-lite への固定は
**乗り換えではなく、すでに動いている実体に正しい名前を付ける**操作にあたる。

なお lite ティアは短い会話プロンプトでは思考トークンを 0 しか使わないため、
`_thinking_off()` が `None` を返して thinking が既定ONになっても
`max_output_tokens=400` は枯渇しない（実測 STOP 5/5・本文最小113字）。チャット側に
`_safe_max_tokens()` 相当は不要。

### 再発防止として効いていないもの / 効くもの
- ❌ 公式 deprecation ページの監視（§1: 「新規ユーザー利用不可」を載せない）
- ❌ `models.list()` の存在確認（`gemini-2.0-flash` は list に載るが実呼び出しは 404）
- ❌ モックのみのユニットテスト（#279 がこれで死んだモデルを通した）
- ✅ **実APIへの疎通確認**（別PRで日次ヘルスチェックcronを追加予定）
- ✅ env 差し替えだけで載せ替えられる構造（本PR）

---

## 1. 発端の前提と、実際に確認できた事実

| 前提（依頼時） | 実際（2026-08-05 実測・公式ページ確認） |
| --- | --- |
| `gemini-2.5-flash` は 2026-10-16 に提供終了 | **誤り**。公式 deprecation ページ（2026-08-03 更新）の該当行は "No shutdown date announced"＝**終了日未定**。後継の指定も無い。10/16 は `gemini-2.5-flash-preview-*` 等の別行と混同した可能性 |
| 後継は `gemini-3.6-flash` | 公式に「`gemini-2.5-flash` の後継」とは書かれていない。ただし `gemini-2.5-flash-preview-05-20` / `-09-25` の Replacement 列は `gemini-3.6-flash`。実在し、当プロジェクトのAPIキーで利用可能 |
| `gemini-2.5-flash-lite` は公称終了日より前に死んでいる | **正しい**。実測で `404 NOT_FOUND … "no longer available to new users"` を再現。**そして公式ページ上ではこのモデルも "No shutdown date announced" のまま** |

### ここが本質的なリスク
公式 deprecation ページは「新規ユーザー利用不可」への切り替わりを**載せない**。
`gemini-2.5-flash-lite` がその実例で、ページ上は「終了日未定」のまま実際には 404 を返す。
**したがって「終了日が未定だから安全」という判断は成り立たない。** 監視源をページだけに置かず、
いつ死んでも env の差し替えだけで復旧できる状態を作っておくことが対策になる。

---

## 2. 実測結果

### 2-1. 依頼された組み合わせ（thinking_budget=512 × max_output_tokens=2048）

**受け付けられる。** 分析ターン（`generate_feedback`）の設定をそのまま `gemini-3.6-flash` に載せ替えて成功。

| モデル | finish_reason | 応答 | thinking トークン | 本文トークン |
| --- | --- | --- | --- | --- |
| `gemini-2.5-flash` | STOP | 5.8s | 509 | 225 |
| `gemini-3.6-flash` | STOP | 7.4s | 673 | 96 |

補足: `gemini-3.6-flash` では thinking トークンが指定予算（512）を**超える**（実測 673）。
予算は上限ではなくヒント。`max_output_tokens` は thinking より十分大きく取る必要があるという
既存コメントの前提は 3 系でも（むしろ強く）成り立つ。

### 2-2. 本当の地雷は `thinking_budget=0`（＝音声パス）

依頼では 512 を懸念していたが、**実際に壊れるのは音声パスの `thinking_budget=0`**。
`classify_register_audio` と `analyze_pronunciation` は両方とも 0 を送っている。

| モデル | tb=0 | tb=128 | tb=512 |
| --- | --- | --- | --- |
| `gemini-2.5-flash` | ✅ | ✅ | ✅ |
| `gemini-3.6-flash` | ❌ **400 INVALID_ARGUMENT** | ✅ | ✅ |
| `gemini-3.5-flash` | ✅ | ✅ | ✅ |
| `gemini-flash-latest`（エイリアス） | ❌ **400** | ✅ | ✅ |
| `gemini-flash-lite-latest`（エイリアス） | ❌ **400** | ✅ | ✅ |

音声パートの有無に関係なく再現する（テキストのみでも 400）。

### 2-3. thinking を切れない世代では出力枠が足りない

`gemini-3.6-flash` は thinking を無効化できないため、思考トークンが `max_output_tokens` を食う。
同一プロンプト・`thinking_budget=128` で N=8 反復:

| max_output_tokens | finish_reason | 本文の最小長 |
| --- | --- | --- |
| 400（＝`llm_max_tokens` の現行値） | **STOP 3 / MAX_TOKENS 5** | 12文字（途中で切れる） |
| 600（＝`classify_register_audio` の現行値） | STOP 8 / 8 | 97文字 |
| 1024 | STOP 8 / 8 | 97文字（thinking 実測最大 670） |

`analyze_pronunciation` は `llm_max_tokens=400` を使っているため、
**モデルIDだけを差し替えると 8回中5回、講評が数十文字で切れる**。600 も thinking 実測 670 に対して余裕が無い。
→ 安全側の下限を **1024** とする。

### 2-4. エイリアスを使わない理由（実証）

`gemini-flash-lite-latest` / `gemini-flash-latest` は、いずれも**すでに Gemini 3 系を指しており**
`thinking_budget=0` を拒否する。エイリアスは「指す実体が予告なく変わり、その時に
呼び出しパラメータの互換性まで壊れる」ことがこれで実証された。
バージョン固定なら、壊れるタイミングをこちら側の都合で選べる。

---

## 3. 実装（このPRの変更）

### 3-1. 設定の追加（`backend/app/core/config.py`）

| 設定 | 既定 | 意味 |
| --- | --- | --- |
| `llm_audio_thinking_budget` | `0` | 音声入力時の thinking 予算。0＝無効（2.5 系のみ可） |
| `llm_audio_max_tokens` | `1024` | 音声入力時の出力枠。従来は 600（声区）/ 400（発音）で分かれていた |

`llm_audio_model` / `llm_analysis_model` のコメントに、バージョン固定の理由・
終了予定日の現況・緊急時の env 差し替え手順を明記した。

### 3-2. モデル世代ガード（`backend/app/coaching/llm.py`）

```python
_THINKING_OFF_SUPPORTED = re.compile(r"^gemini-2\.\d", re.IGNORECASE)
_MIN_THINKING_BUDGET = 128
_MIN_MAX_TOKENS_WITH_THINKING = 1024

def _safe_thinking_budget(model, budget) -> int   # 0 指定を、切れない世代では 128 へ引き上げ
def _safe_max_tokens(model, budget, want) -> int  # 思考が有効なら出力枠の下限を確保
```

**狙いは「緊急時に再デプロイを待たないこと」**。
`LLM_AUDIO_MODEL=gemini-3.6-flash` を env に置いて再起動するだけで載せ替わる。
ガードが無いと、その差し替えは 400 で失敗し、コード修正＋デプロイが必要になる。

既定の `gemini-2.5-flash`（tb=0）では従来どおり thinking 無効・挙動もコストも変わらない。
回帰テスト: `backend/tests/test_llm_model_generation_guard.py`（17件）。

### 3-3. 既定モデルは据え置き

`gemini-2.5-flash` のまま。理由は §4 のとおり、音色の聞き分け品質の before/after 比較が未完のため。
コード側は env だけで載せ替えられる状態になっている。

---

## 4. 品質比較の状況

### 4-1. 分析ターン（`generate_feedback`）— 比較済み・実用差なし

合成音＋実測相当の Evidence Pack で N=6:

| モデル | 失敗 | `**bold**` 混入 | 見出し | 箇条書き | 絵文字 | 文字数中央値 | 応答中央値 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `gemini-2.5-flash` | 0/6 | 2/6 | 0/6 | 0/6 | 2/6 | 407 | 5.7s |
| `gemini-3.6-flash` | 0/6 | 0/6 | 0/6 | 0/6 | 1/6 | 304 | 8.3s |

両モデルとも `llm_analysis_timeout_sec=90` に対して十分速い。3.6 の方がやや短く、やや遅い。

> **別件で見つかった既存バグ**: `**bold**` が **現行の 2.5-flash で 6回中2回** 混入する。
> フロントは `whitespace-pre-wrap` の素テキスト描画（`frontend/src/components/coach/Bubbles.tsx`）なので、
> `**リップロール**` がそのまま画面に出る。`ANALYSIS_SYSTEM_PROMPT` は「Markdown**見出し**は使わない」としか
> 言っておらず、太字を禁じていない。3.6 への移行とは独立した問題。docs/42 の管轄。

### 4-2. 音色の聞き分け（`classify_register_audio`）— 実録音で比較済み

#### 使用サンプル
本番 `uploads/coach` にあった実録音1件（運用者自身のもの・本人確認済み）。
16.62秒 / 22.05kHz / F0中央値 437Hz・95パーセンタイル 864Hz（高音を含む歌唱）。
検証後、ローカルのコピーは削除した。

> 合成音では代用できないことが事前に判明している。
> `gemini-2.5-flash` は合成音に対し「裏声で歌われている」と息の混じり・厚みまで描写して断定した
> （**実在しない声区の作話**）。`gemini-3.6-flash` は「人間の歌声ではなくシンセサイザーによる電子音」と
> 正しく見抜き、判定自体を拒否した。

#### 自己一貫性（同一録音・同一プロンプトで N=8 反復・dsp_hint なし）

| モデル | 粗い判定（地声 / ミックス / 裏声） | 細かい寄り | 失敗 | 応答中央値 |
| --- | --- | --- | --- | --- |
| `gemini-2.5-flash` | **地声 3 / ミックス 5**（ブレる） | 裏声寄り 2・明示なし 3 | 0/8 | 2.9s |
| `gemini-3.6-flash` | **ミックス 8/8**（一致） | 地声寄り 4・裏声寄り 2・明示なし 2 | 0/8 | 9.6s |

**判定は落ちていない。むしろ粗い判定の安定性は明確に改善した。**
ユーザーが実際に聞くのは「これミックス？」という粗いレベルの問い（`coach.py` の声区判定経路）であり、
現行モデルはそこで 8回中3回 別の答えを返す。3.6 は 8/8 で一致した（別途 N=6 でも 6/6 ミックス）。

現行モデルは記述の内部矛盾も出た（同一音声に対し、ある回は「息が多めに混ざっている」、
別の回は「息の混じり方も少なく」）。3.6 は「息漏れが少ない・倍音が豊か・芯がある」で回をまたいで安定していた。

細かい寄り（地声寄り／裏声寄り）は 3.6 でもブレる。ここは元々プロンプトが
「1つに断定しづらければ『ミックス（地声寄り／裏声寄り）』でOK」と許容している範囲。

#### トレードオフ: レイテンシ

`classify_register_audio` は `backend/app/api/v1/endpoints/coach.py:254` で
**リクエスト処理中に同期呼び出しされ、待ち時間の打ち切りが無い**（録音FBのコーチコメントにある
`llm_coach_wait_sec` のような仕組みが無い）。したがって応答時間はそのままユーザーの待ち時間になる。

- `gemini-2.5-flash`: 中央値 2.9s（実測レンジ 2.3〜3.6s）
- `gemini-3.6-flash`: 中央値 9.6s（実測レンジ 4.8〜13.0s、別ランで最大 **33.9s**）

`llm_audio_timeout_sec = 60.0` の範囲には収まるので機能は壊れないが、
**声区を聞くたびに10秒前後、最悪30秒超待たされる**のは体験として無視できない。
既定を 3.6 に切り替えるなら、この経路にローディング表示か待ち時間の打ち切りを入れるべき。

---

## 5. 結論と残タスク

### 結論
`gemini-3.6-flash` への移行は**品質面では問題ない**（判定の安定性はむしろ向上）。
唯一の障害はレイテンシ（声区判定で 2.9s → 9.6s、最悪 33.9s）で、これは UI 側の手当てが要る。
そのため本PRでは**既定を `gemini-2.5-flash` のまま据え置き**、
「いつでも env 1行で載せ替えられる状態」を作るところまでを実装した。

### 残タスク
1. 声区判定経路（`coach.py:254`）のローディング表示／待ち時間の打ち切り → 対応後に既定を 3.6 へ切替
2. チャット経路（`llm_model` / `llm_chat_model` = `gemini-flash-lite-latest`）が
   `thinking_budget=0` を送っており 400 で拒否される件 → **別PRで最優先対応**。
   例外は握り潰されるため、ソラ先生のチャット返答が無言でルールベースに落ちている疑い。
   `gemini-flash-lite-latest` は thinking を有効化すると実測 45.7 秒で `llm_timeout_sec=20` を超えるため、
   モデルIDのバージョン固定込みの判断が要る（docs/66 の決定に触れる）
3. `**bold**` 混入（4-1 の注記）→ docs/42 側で扱う

## 5.5 続報: 「迷ったらミックス」の逃げ道除去と再ベンチ（2026-08-08）

### 発端（運用者指摘）
§4-2 の「全モデルがミックスで一致」に対し、運用者から「迷ったときにミックスと言うのはやめてほしい」
という決定が出た。調査すると、**本番プロンプト自体に「1つに断定しづらければ『ミックス（地声寄り／
裏声寄り）』と答えてOKです」という逃げ道が書かれていた**。つまり観測された一致は、
モデルが同じ判定に達したのではなく、**不確実な時に全員が同じ逃げ場へ収束していただけ**の疑いが強い。

### 逃げ道を外して再ベンチ（VPSコンテナ内・実録音・各N=8・否定文脈を除外した判定抽出）

| モデル | 応答中央値 | 最悪 | 判定分布 |
| --- | --- | --- | --- |
| gemini-2.5-flash（現行） | ~2.5s | — | 地声と裏声を箇所別に描写 |
| gemini-3.6-flash | 5.4s | 8.2s | 裏声4 / 両方に言及2 / 地声2 |
| gemini-3.1-flash-lite | 2.7s | 3.9s | 地声5 / ミックス2 / 裏声1 |
| gemini-3.5-flash | 5.2s | 6.1s | 両方に言及5 / 地声2 / 裏声1 |

**結果: 逃げ道を外すと、モデル内でも・モデル間でも判定が割れた。** §4-2 の「3.6は8/8一致」は
プロンプトが作った見かけの一致であり、撤回する。

重要な観察: 複数モデルが「0:05〜0:10あたりは地声、0:11あたりは裏声」のように**箇所によって
声区が違う**と描写した。検証録音（F0中央値437Hz・95%ile 864Hz）自体に複数の声区が含まれて
いる可能性が高く、「特に高い音の箇所はどれか」という問い自体がこの録音では多義的。
判定の割れは、少なくとも部分的には**録音の実態を正直に反映したもの**と考えられる。

### 決定と実装（2026-08-08）
1. **逃げ道の削除**（運用者決定）: プロンプトを「聴こえたとおりに判定。迷った時の無難な答えとして
   『ミックス』を使わない。確信が持てなければ、どの特徴がどちらに聴こえるかを正直に言う」に変更。
2. **毎回その場で聴く**（運用者決定）: 事前計算・キャッシュ・ヘッジ（保険の並走呼び出し）はいずれも
   採用しない。「ちゃんと聴いた上で判断する」体験を維持する。
3. **前回判定メモリ**: 声区の返答を `ChatMessage.payload`（`register_reply` マーカー）で記録し、
   次回の判定時に前回の判定文と「同じ録音か別テイクか」をプロンプトに渡す。
   - 同じ録音への再質問 → 聴き直しを指示。前回への追従は不要と明示（異議に対して同文即答を返さない）
   - 別テイク → 判定が変わる場合は「前回は〜でしたが」と**変化として触れる**（黙って逆を言わない）
   - どちらも「前回に引きずられて判定を曲げない」を明示（メモリは口ぶりのためで、判定を汚さない）
4. **モデル既定は 2.5-flash 据え置き（継続）**: 逃げ道なしの条件ではモデル間の判定が割れており、
   正解ラベル無しでは優劣を決められない。運用者が用意する**正解付き録音**（地声のみ・裏声のみ・
   ミックスのつもり、と分かって歌ったもの）での精度測定を待って最終判断する。
   レイテンシ面の参考: 3.6 は VPS から中央値 5秒前後（§4-2 の 9.6s は Mac からの測定で過大だった）。

### 教訓
- 「一貫している」は「正しい」を意味しない。プロンプトの逃げ道は、無難な誤判定への収束を
  一貫性に見せかける。一貫性の測定は、逃げ道を塞いでから行うこと。
- ベンチの判定抽出は否定文脈に注意（「裏声特有の軽さは**感じられません**」を「裏声」と
  誤カウントしていた）。

## 5.6 正解付き録音での精度測定 → llm_audio_model を gemini-3.5-flash に切替（2026-08-08）

### 測定
運用者本人が「地声のみ」「裏声のみ」「ミックスのつもり」と分かって歌い分けた3本
（本番アプリ経由・本人アカウント確認済み・DSPでラベルの物理的整合も確認: 裏声はF0が高く
スペクトル重心が低い＝典型的特徴）。本番コードパス（逃げ道なしプロンプト・dsp_hint なし）で
各モデル×各録音 N=6。VPSコンテナ内で実施し音声はサーバー外に出していない。

| モデル | 地声 | 裏声 | ミックス | 総合 | 応答中央値 |
| --- | --- | --- | --- | --- | --- |
| gemini-2.5-flash（旧既定） | **0/6**（全て裏声と誤答） | 2/6 | 0/6 | **2/18** | ~3s |
| gemini-3.6-flash | 0/6 | 3/6 | 2/6 | 5/18 | ~6s |
| gemini-3.1-flash-lite | 0/6（全て裏声と誤答） | 6/6 | 0/6 | 6/18 | ~3s |
| **gemini-3.5-flash** | **5/6** | **5/6** | 0/6 | **10/18** | 7〜12s |

### 結論
- **地声と裏声を聞き分けられるのは gemini-3.5-flash だけ**（10/12）。他の3モデルは
  地声の録音をほぼ全て「裏声」と答えた。3.1-flash-lite の「裏声6/6」は全部裏声と答える
  モデルだから当たっただけ（地声も全部裏声と誤答）。
- **ミックスは全モデル 0/6**。ただし 3.5-flash は「地声の芯と裏声の軽さの両方がある」と
  特徴を正直に描写しており（両方に言及3/6）、誤答の中では最も誠実。ミックスの断定精度は
  現状のLLMでは期待できないことを、コーチング文面設計の前提とすべき。
- **決定**: `llm_audio_model` 既定を **gemini-3.5-flash** に変更（運用者事前合意の
  「正解付き録音で勝ったモデルに切替」に基づく）。レイテンシは約3秒→7〜12秒に増えるが、
  「3秒で確実に間違う」より「10秒で当たる」を選ぶ。「考えています」ドット表示あり。
  `analyze_pronunciation`（発音FB）も同じ設定を共有するため同時に 3.5-flash となる。
- 注意: n=1人・3テイクの測定。歌い手が変われば傾向が変わる可能性はある。ただし
  「本人の地声を全モデルが裏声と誤答する」旧状態より悪化する余地は小さい。

## 5.7 声区判定の設計を「単一ラベル」から「換声点のつながり」へ（2026-08-08・運用者指摘）

### 発端: 前提の誤り
運用者から「**ミックスボイスとは地声と裏声が混ざったというよりも、地声の筋肉と裏声の筋肉が
スムーズにつながっている状態**」との指摘。発声科学的に正しく、§5.6 までの設計（および
§5.6 で一度採用しかけた「混ざり具合の描写」案）は**俗説寄りの前提に立っていた**。

正しい機構（voice-scientist 諮問）:
- 地声＝TA（甲状披裂筋）優位で声帯が厚く振動 / 裏声＝CT（輪状甲状筋）優位で薄く伸展
- **ミックス＝音域移動に伴い TA優位→CT優位へ支配が滑らかに移行し、換声点(passaggio)で
  段差が出ない「協調状態」**。2つの音色のブレンドではない。
- ゆえに **1つの音を切り取って「これはミックスか」は機構的に答えの出ない問い**。
  協調は「音域を動いた時に段差が出るか」にしか現れない。
- 音響から取れる H1-H2・スペクトル傾斜・シンガーズフォルマント比は内転度と共鳴の指標であり、
  TA/CT の活動比を直接示さない。chest↔mix は音響的に連続で境界を引けない。

**§5.6 の「ミックス的中 0/6」の解釈を訂正**: モデルの性能限界ではなく、
**問いの設計が機構と噛み合っていなかった**（answerable でない問いを投げていた）。

### 実装（問いの作り替え）
「どの声区か」→「換声点がどうつながっているか」に変更:
- **音域移動あり** → 段差の有無と位置を答える。段差があれば見立てを処方に直結
  （手前まで芯が強くそこで破綻＝pulled chest → サイレン/リップトリル、
   段差の後で急に息っぽく細くなる＝薄い裏声へ逃げ → ネイ/ギで前に当てる）
- **単一音域のみ** → つながりは判定不能と正直に伝え、音の状態の描写にとどめ、
  サイレン録音へ誘導
- 「これミックス？」には定義を一言添えて生徒の理解も正す

### 重要な失敗と修正: 音域移動の有無をモデルに聴かせてはいけない
最初の実装では「音域移動が含まれるかを聴き取ってください」とモデルに判断させた。
実測（gemini-3.5-flash・実録音3本）で**結果が正反対**になった:

| 録音（実測f0） | モデルに聴かせた場合 | DSP実測を渡した場合 |
| --- | --- | --- |
| 地声・単一音域 | 「段差あり。裏声へ逃げている」と誤診 | 「単一音域なので判定できません」＋誘導 |
| 裏声・単一音域 | **「段差なく滑らかにつながっています。これが理想的な状態です」** | 「移動がないので判定できません」＋条件付き助言 |
| 2.5オクターブ移動 | 「この音だけでは断定できません」（唯一測れる録音を取りこぼし） | 「0:01で地声から急にひっくり返る段差」→pulled chest→サイレン処方 |

**特に危険だったのは2行目**: 純粋なファルセット録音を「ミックスの理想形」と褒めており、
生徒の「裏声へ逃げる癖」を強化してしまう。

f0 は DSP で正確に測れているので、聴かせるのではなく**事実として渡す**
（`llm.build_range_hint()`: f0中央値＋伸ばし区間の平均f0から最低/最高を取り、
長6度=0.75オクターブ以上で「移動あり」と判定。docs/42 §5「事実はDSP、判断はモデル」の原則どおり）。

### 教訓
- 前提（対象の定義）が誤っていると、測定・モデル選定・プロンプトのすべてが的外れになる。
  §5.6 のモデル比較は「答えの出ない問い」での比較だった。
- **モデルに聞くべきこと（音色の質感）と、測って渡すべきこと（f0・音域）を混ぜない。**
  測れるものを聴かせると、当たっているように見えて正反対の誤診を生む。

## 6. 監視について

公式 deprecation ページは「新規ユーザー利用不可」を載せないため、単独の監視源としては不十分。
実質的な早期警戒は「本番から実際に呼んでみて 404/400 が返るか」しかない。
本PRのガードは、その事象が起きた後の**復旧を env 1行＋再起動に縮める**ことを目的としている。

## 5.8 段差判定は聴覚では不安定 → 断定を停止し DSP 検出へ（2026-08-08）

### 測定（運用者提供・正解ラベルあり）
運用者本人が「クリアに声区融合したサイレン」として録音（14.7秒・実測 2.25オクターブ
＝D2〜F4付近）。正解は「段差なし」。本番プロンプト（§5.7）で各モデル N=6:

| モデル | 段差なし（正解） | 段差あり（誤答） | 両論併記 | 不明 | 応答中央値 |
| --- | --- | --- | --- | --- | --- |
| gemini-3.5-flash | 3/6 | 1 | 1 | 1 | 11.2s |
| gemini-3.6-flash | 4/6 | 1 | 1 | 0 | 10.0s |

先行して測った別のサイレン（同一話者・正解ラベルなし）では、3.5-flash が「段差あり」4/6、
3.6-flash が「段差なし/両論」6/6 と**モデル間で結論が逆転**していた。

### 結論: 聴覚だけの段差判定は断定に使えない
- 正解が「段差なし」と分かっている録音でも、両モデルとも **6回中2〜3回は正解を返せない**。
- 誤断定の害は双方向に大きい:
  - 融合できている人に「段差があります・地声を引っぱりすぎです」とダメ出しし、不要な練習を処方する
  - つながっていない人を「段差なく滑らかです。**これが理想的な状態です**」と褒め、癖を強化する
- 同一モデル・同一録音の連続実行で正反対の答えが出る（前回判定メモリでは救えない）。

### 暫定対応（本PR）
段差の有無を**断定させない**。「〜秒あたりで音色が軽くなっていくのが聴こえます」のように
観察の共有にとどめ、「理想的な状態です」の褒めと、段差判断を根拠にした練習処方を禁止する。

### 恒久対応（次PR）: DSP による段差検出
段差は物理的に観測可能なはずで、f0 上昇中の音色指標（スペクトル傾斜・H1-H2・
シンガーズフォルマント比）の**時間変化の不連続**として現れる。音域移動の有無を
DSP に移して成功した（§5.7）のと同じ構図。運用者提供の2本のサイレンを正解ラベルとして
検証する。

### 付随して修正した欠陥: サイレンで音域が取れない
`build_range_hint` は伸ばし区間から音域を取っていたが、**サイレンは連続的に上下降するため
伸ばし区間が0個**検出され、None を返していた（＝つながり判定の本命入力で機能しない）。
解析に `f0_low_hz` / `f0_high_hz`（有声フレームの5/95パーセンタイル）を追加し、
伸ばし区間が無くても音域を取れるようにした。古い解析には無いので後方互換の
フォールバック（中央値＋伸ばし区間）を残してある。

## 5.9 換声点の段差を DSP で検出（2026-08-08）

### 材料（運用者提供・正解ラベル付き）
| 録音 | 正解 | 実測音域 |
| --- | --- | --- |
| サイレン1 | 不明 | 2.36oct（70〜359Hz） |
| サイレン2「クリアに声区融合」 | **段差なし** | 2.25oct（75〜356Hz） |
| サイレン3「意図的に段差を作った」 | **段差あり** | 2.75oct（91〜607Hz） |

### 設計
段差は「時間」ではなく**音高**の現象。音色指標を f0（半音ビン）の関数として並べ、
**半音あたりの変化率の不連続**を見る。上昇・下降で同じ音高を複数回通るぶんは中央値で集約し、
半音が連続している最長区間だけを使う（欠損を跨いだ差分は段差ではない）。

**主指標はスペクトル傾斜のみ。** H1-H2 も声区で動くが、実測のプロファイルでは母音・マイクの
影響で半音ごとに **±10〜29dB** 振れ、段差検出には使えなかった。当初は「2指標の合意」を
必須にしていたが、H1-H2 のノイズが合意を破壊し、正解の段差すら検出できなかった。

判定は **z（相対）と dB/oct（絶対）の両方**を満たした時のみ:
- ロバストz（MAD正規化）≥ 4.0 … その録音の通常の揺らぎに対する外れ度
- |Δ傾斜| ≥ 5.0 dB/oct … 物理的な大きさ。zだけだと揺らぎの小さい録音で微差を段差と誤判定する
- 候補は 150Hz 以上・端2ビンを除外（立ち上がり/切れ際の乱れを拾わない）
- 連続音域が10ビン未満なら **None＝判定しない**（断定させない安全側）

### 結果（`analyzer.detect_passaggio_step`）
| 録音 | 正解 | 判定 | z | Δ傾斜 |
| --- | --- | --- | --- | --- |
| クリア融合 | 段差なし | ✅ has_step=False | 3.51 | -2.9 dB/oct |
| 意図的な段差 | 段差あり | ✅ **has_step=True** C#4(277Hz) | **7.02** | **-10.3 dB/oct** |
| サイレン1 | 不明 | has_step=False | 1.30 | -2.3 dB/oct |

正解2本とも的中し、分離も大きい（z 7.02 vs 3.51、Δ -10.3 vs -2.9）。
聴覚判定（§5.8: 6回中2〜3回誤答）と違い、**同じ入力には常に同じ答えを返す**（決定的）。

### 実装
- `analyzer.detect_passaggio_step()` → 解析結果に `passaggio` を追加
- `llm.build_passaggio_fact()` が事実の1行に変換し、プロンプトへ注入
- **事実がある時だけ**断定と処方を許可（§5.8 の暫定停止を解除）。
  向き（`to_head`＝軽くなる／`to_chest`＝厚くなる）で処方を分岐:
  軽くなる＝pulled chest →サイレン/リップトリル、厚くなる＝裏声から落として繋ぐ →ネイ/ギ。
- DSP が判定できない時（音域が狭い等）は `None`＝従来どおり断定させない。

### 実LLMでの通し確認
- クリア融合 → 「換声点における音色の段差は検出されませんでした。…段差なく一本につながった
  状態こそが、まさにミックスボイスで目指している理想的な声区の融合です」
- 意図的な段差 → 「11.08秒あたり（C#4付近）で音色が急に軽く裏声寄りに跳ぶ不連続な段差が
  検出されました。これは…プルド・チェストと考えられます」→ サイレンの練習を処方

### 限界（正直な記録）
- **正解ラベルは1人・2本のみ。** 閾値（z=4.0 / 5.0dB/oct）はこの2本で分離するよう決めた値で、
  他の歌い手・他のマイクで同じ閾値が最適である保証は無い。
- 検出できるのは「音色の急変」であって、TA/CT の活動比そのものではない。
- 誤検出時の害は §5.8 のとおり双方向に大きいので、閾値を緩める方向の変更は
  必ず正解ラベル付きの録音を増やしてから行うこと。

### 付随して起こした障害（反省）
検証のため**本番バックエンドコンテナ内で librosa.pyin を繰り返し実行**した結果、
メモリ960MBのVPSで起動時の numba JIT がセグメンテーションフォルトを起こし、
バックエンドが再起動ループ＝**API が 502 で停止**した（`docker compose up -d --force-recreate`
で復旧）。本番コンテナを計算資源として使わないこと。音声はローカルへ取得して検証し、
終わったら削除する（本作業でもそうした）。

## 5.10 llm_audio_model の再選定 — 3.5-flash → 2.5-flash に差し戻し（2026-08-08）

### なぜ測り直したか
§5.6 のモデル比較は「単一ラベルで声区を当てられるか」で行ったが、**その問い自体が機構的に
無効だった**（§5.7）。段差判定を DSP に移した（§5.9）ことでモデルの仕事は変わり、いまは
「**実測の判定に従って音色を描写し、向きに合う練習を1つ添える**」である。
声区を聞き分ける能力はもう要求されない。したがって選定基準は
「実測への追従」と「応答速度」になる。

### 測定（正解付きサイレン2本・DSPの事実を注入した本番プロンプト）
| モデル | 段差あり | 段差なし | 応答中央値 | p90 | 最大 |
| --- | --- | --- | --- | --- | --- |
| **gemini-2.5-flash** | 12/13 | 13/13 | **3.1s** | 4.1s | **4.8s** |
| gemini-3.1-flash-lite | 12/13 | 13/13 | 2.6s | 3.5s | 3.9s |
| gemini-3.5-flash（当時の既定） | 5/5 | 5/5 | 2.1s | 21.3s | **21.3〜49.4s** |
| gemini-3.6-flash | 5/5 | 5/5 | 7.5s | 8.7s | 8.7s |

**DSPが判定を供給するようになった結果、どのモデルもほぼ完璧に追従する**（差が消えた）。
残る差は速度だけになり、3.5-flash は**尾が極端に重い**（最大 21〜49秒）ことが分かった。

### 決定
`llm_audio_model` を **gemini-2.5-flash** に差し戻す。
- 応答が安定して速い（中央値 3.1s・最大 4.8s）。§5.6 で「7〜12秒に伸びる」と了承を得た
  トレードオフが**そもそも不要になった**
- `analyze_pronunciation`（発音FB）でも長年の本番実績がある既知の品質
- 3.1-flash-lite は 0.5秒速いが、lite ティアでの**発音の聞き取り品質が未計測**のため既定にしない
  （速度を最優先する判断になれば候補）

### 測定の途中で自分が犯した誤り（記録）
第1回の集計で「実測と矛盾 1/4」と出たが、応答の実物を確認したところ**全て正しく追従していた**。
判定用の正規表現が「段差は検出され**ず**」の否定を認識できず、矛盾として数えていた。
**スコアだけを見て結論を出していたら、正しく動いているモデルを不当に落としていた。**
自動集計の異常値は、実物を見るまで信じないこと。

### 注意（世代リスク）
2.5 系は同世代の `gemini-2.5-flash-lite` が「新規ユーザー利用不可」で 404 死した前例がある
（§1）。`gemini-2.5-flash` 自体は公式ページ上「終了日未定」だが、同じ経路で死ぬ可能性はある。
日次ヘルスチェック（§0・`scripts/ops/gemini_model_healthcheck.py`）が翌朝に検知し、
`docker/.env` の `LLM_AUDIO_MODEL` 1行で載せ替えられる。差し替え先の第一候補は
**gemini-3.1-flash-lite**（本節の実測で同等の追従・最速）。
