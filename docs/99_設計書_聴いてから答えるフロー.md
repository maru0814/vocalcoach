...
> **改訂 2026-08-08（(A)案）**: 専用タグUI・`user_tag` フィールドは撤去し、申告は既存コメント欄（`comment`）の自然言語で受ける（docs/95 FR-03 改訂・docs/98 参照）。

対応要件: docs/95（FR-01〜04）／ UI仕様: docs/98
...
### frontend（docs/98 準拠）
- `Composer`: placeholder を申告誘導文言に変更するのみ（docs/98）。API・props の変更なし。
