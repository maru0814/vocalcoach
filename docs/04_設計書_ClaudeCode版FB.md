...
```mermaid
flowchart TD
    U[ユーザー] -->|起動| PA[Phase A: 情報収集]
    PA -->|録音/区間/原曲/原曲区間/観点| PB[Phase B: 解析と課題発見]
    PB -->|analyzer 実行| AN[(audio_analyzer.py)]
    AN -->|JSON| PB
    PB -->|課題1つ特定| PC[Phase C: 基礎練処方+参考動画]
    PC -->|WebSearch| WS[(YouTube)]
    WS -->|動画URL| PC
    PC -->|録音依頼| PD[Phase D: 基礎練の確認]
    PD -->|analyzer 単体実行| AN
    PD -->|未達(△/✗)| PC
    PD -->|達成(◯)| PE[Phase E: 曲再録音判定]
    PE -->|analyzer 比較実行| AN
    PE -->|改善あり| PB
    PE -->|改善なし| PB
    PE -->|完了通知| U
...
- **コーチングループ採用**: 単発FBではなくループ化したことで、ユーザーは複数回のやり取りが必要になる（=ユーザー側の負荷増）。引き換えに、独学では難しい「課題切り分け→処方→検証」の伴走が可能になる。1回完結を望むユーザーには Phase B 出力のみで終わるショートカットを提供する
- **状態管理は会話コンテキスト依存**: ループのフェーズや過去解析結果はファイル等で永続化せず、Claude の会話コンテキストに保持する。セッション切れ時は Phase A から再開（ユーザーには直近録音と課題の再共有を促す）
...
#### 4.5.6 時間範囲指定
- CLI 引数 `--user-start`, `--user-end`, `--ref-start`, `--ref-end` で mm:ss または秒数を受け付ける
- 内部実装は `librosa.load(offset=, duration=)` で必要区間だけをデコード（全体読み込み回避で高速化）
- 出力JSONに `user_range` / `reference_range` を含め、解析対象範囲を明示する
...
stateDiagram-v2
    ...
    D --> C: 未達(△/✗)→別アプローチ
    D --> E: 達成(◯)
    E --> B: 改善あり→次の課題 / 改善なし→課題切り分け直し
    E --> [*]: ユーザー終了宣言
```

#### 4.6.2 状態保持の方針
- 状態はファイル等で永続化せず、Claude の会話コンテキストに保持
- 各遷移時に保持する変数:
  - `current_song`, `user_audio_path`, `ref_audio_path`
  - `user_range`, `ref_range`
  - `baseline_analysis` (Phase B の初回解析結果、Phase E で比較に使う)
  - `current_task` (タクソノミーから選んだ課題ID)
  - `phase_history` (どのフェーズを通ったか、Phase D の試行回数）

#### 4.6.3 課題タクソノミーの実装
- SKILL.md 内の表として保持（実装ファイル不要、Claude が表を読んで判断）
- 各課題に対する基礎練・達成判定基準・WebSearch クエリを表に列挙
- 新規課題の追加時は SKILL.md の表に行追加するだけで反映

### 4.7 参考動画の選定パイプライン
1. `WebSearch` ツールで課題ごとのクエリを実行
2. 上位5件から以下のフィルタを適用:
   - 再生数（取得可能なら）が一定以上
   - タイトル/サマリに解剖学的説明の手がかり（横隔膜・声帯・軟口蓋などのワード）
3. 1-3本を Phase C 出力に貼る
4. 候補が空なら別クエリで再検索
5. 動画が見つからなくても基礎練の文章説明は必ず提示
...
sequenceDiagram
    ...
    Note over S,U: Phase A: 情報収集
    U->>S: 起動依頼
    S->>U: 5項目（録音/区間/原曲/原曲区間/観点）を要求
    U->>S: 5項目を提供
    Note over S,A: Phase B: 解析と課題発見
    Note over S: 原曲がURLの場合は yt-dlp で wav 抽出
    S->>A: audio_analyzer.py 起動 (区間指定)
    A->>S: 解析JSON (user/ref/compare)
    Note over S: 同一音源検出時は中断し再提出を要求
    S->>S: 課題タクソノミーにマッピング、課題1つ特定
    S->>U: Phase B 出力 (解析データ+課題)
    Note over S,W: Phase C: 基礎練処方
    S->>W: 課題別クエリで参考動画検索
    W->>S: 動画候補
    S->>U: 基礎練メニュー + 参考動画URL
    Note over S,A: Phase D: 基礎練の確認
    U->>S: 基礎練録音
    S->>A: audio_analyzer.py 起動 (単体解析)
    A->>S: 解析JSON
    S->>S: 達成判定 (◯/△/✗)
    alt 未達(△/✗)
        S->>U: 別アプローチを提示 (Phase C に戻る)
    end
    Note over S,A: Phase E: 曲再録音判定
    U->>S: 曲の再録音
    S->>A: audio_analyzer.py 起動 (初回と比較)
    A->>S: 解析JSON
    S->>S: 初回との差分計算
    alt 改善あり
        S->>U: 褒める + 次の課題を Phase B から選択
    else 改善なし
        S->>U: Phase B に戻り課題切り分けやり直し
    end
```
...
sequenceDiagram
    actor U as ユーザー
    participant S as vocal-trainer skill
    Note over S: 前回コンテキスト消失
    U->>S: 再開依頼
    S->>U: 直近の状態を再共有依頼<br/>(録音ファイル / 直前の課題 / Phase D-E のどこまで進んだか)
    U->>S: コンテキスト再共有
    S->>S: 該当 Phase から再開
```
...
| 時間範囲が音源長を超える | 解析を中断し再入力を促す | 「指定された区間 1:30-2:00 は録音長（1:25）を超えています。範囲を見直してください」 |
| 時間範囲のフォーマット不正 | 解析を中断し再入力を促す | 「区間は mm:ss 形式（例: 0:30）でご指定ください」 |
| Phase D で同じ未達が3回続く | Phase B に戻り課題切り分け直し | 「同じ基礎練で達成が見えないので、別の課題から取り組み直します」 |
| WebSearch で参考動画が見つからない | 動画なしで基礎練の文章説明のみ提示 | 「参考動画は見つかりませんでしたが、以下の手順で進めてください: ...」 |
