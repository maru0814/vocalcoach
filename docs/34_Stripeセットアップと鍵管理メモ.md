...
> 将来の自分用。有料プラン（プレミアム ¥50/月・税込。2026-09-16 v4 で ¥800→¥50 に改定）の決済まわり。docs/31〜33・PR-A/Cの続き（PR-B）で使う。
> ⚠️ 価格の正本は Stripe 本番モードの Price 金額。docs・UI・特商法ページの表記は必ず実 Price 金額（現行 ¥50）に一致させる。
...
| `STRIPE_PRICE_ID_PREMIUM` | `price_...` | 商品 → ¥50/月の価格 | サブスク作成時のプラン指定 |
...
- [ ] 本番モードで ¥50/月 の商品を**作り直す**（テストのPrice IDは本番で使えない）
...
### 5.3 本番の商品（¥50/月）を作成
- 商品カタログ → 商品 → 「+ 商品を追加」
- 名前=`ソラ先生 プレミアム`、料金=**50 JPY**、**継続（月次）**
- 発行された `price_...`（本番）を控える。
- ⚠️ Stripe の日本円最低請求額は **¥50**。これ未満の Price を作っても加入時の決済が `amount_too_small` で失敗する（[Stripe docs](https://docs.stripe.com/currencies#minimum-and-maximum-charge-amounts)）。

### 5.3a 価格を改定するとき（例: ¥800 → ¥50、2026-09-16 実施）
Stripe の Price は金額を後から編集できない。**新しい Price を作って差し替える**。
1. 商品カタログ → 既存商品「ソラ先生 プレミアム」→ 「+ 別の料金を追加」→ **50 JPY・継続（月次）** → 保存。新しい `price_...` を控える。
2. 新 Price を「デフォルト」に設定し、旧 ¥800 Price は「アーカイブ」（削除は不可。既存サブスクが参照しているため）。
3. VPS の `docker/.env` の `STRIPE_PRICE_ID_PREMIUM` を新 `price_...` に書き換え → backend 再起動（`docker compose -f docker-compose.prod.yml up -d backend`）。**新規加入はここから ¥50 になる。**
4. コード側 `frontend/src/lib/pricing.ts` の `PREMIUM_PRICE_JPY` と docs/31 を同じ金額にしてデプロイ（表記と実課金額のズレは重大インシデント）。3 と 4 の順番はどちらが先でもズレる時間が生まれるので、**同じタイミングで**やる。
5. **既存加入者は旧 Price のまま**更新され続ける（自動では移らない）。下げる場合: 顧客 → 該当サブスク → 「サブスクリプションを更新」→ 料金を新 Price に変更 → 「日割りしない」を選んで保存。次回更新から ¥50 になる。据え置くか下げるかは docs/31 §10 Q4。
