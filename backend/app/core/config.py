...
class Settings(BaseSettings):
    ...
    # 測定事実（質感・音程移動幅）をブラインド聴取のプロンプトへ注入するか。
    # 既定OFF: 実録音較正第1・2号（docs/106 §4）で、モデルの素の聴覚は10/10正解なのに
    # 事実文の注入がそれを上書きして誤認させることが判明したため（どんな言い回しでも
    # テキストが聴覚に勝ってしまう）。計測とログは注入OFFでも続ける（eval・観測用）。
    # 実録音evalで注入が正答率を上げると実証されたら env で再ON（docs/104 改訂その3）。
    blind_listen_inject_facts: bool = False
