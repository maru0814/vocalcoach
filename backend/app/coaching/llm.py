...
# 分析ターンの詳細な講評作法は ANALYSIS_SYSTEM_PROMPT が別途持つ（経路別台本 [FB-042-02.2-4]）。
# docs/42 来歴（この台本本文が実装するルール。行の対応は本文の節見出しに準ずる）:
#   人物像: [FB-042-01-4]（用語は一言補足）[FB-042-02.2-5]（長文レファレンス撤去・言い換えのみ維持）
#   会話の作法: [FB-042-02-4]（2〜4文・120字）[FB-042-02.1-1]（相槌は軽く受ける）
#     [FB-042-02.1-2]（感情はまず共感）[FB-042-02.1-3]（同じ定型を繰り返さない）
#     [FB-042-02.1-4]（求められたらコーチング）[FB-042-06-1][FB-042-06-2]（練習を続けさせる/切替条件）
#     [FB-042-04-10]（同趣旨を繰り返さない）[FB-042-04-11]（自己引用は実在時のみ）
#     [FB-042-08-8]（安全例外は最優先）
#   事実への忠実さ: [FB-042-02.2-2]（3原則）[FB-042-02.2-3]（禁止形は安全のみ）
#     [FB-042-05-1][FB-042-05-2]（数値・録音長超の秒数を作らない）[FB-042-06-5]（生の数値を出さない）
#     [FB-042-05-3]（照合なしで正確さを断定しない）[FB-042-05-4]（声区は推定・感覚と両立）
#     [FB-042-05-5]（否定は橋渡し付き）
#   レッスンの決まりごと: [FB-042-02-1]（カード無し・会話完結）[FB-042-04-8]（再録音を答えの代わりにしない）
#     [FB-042-04-9][FB-042-08-6]（動画オファーの枠と実在URL限定）[FB-042-05-6]（曲名は検索して確認）
#   出力: [FB-042-02-2]（Markdown禁止。コード側ガードは scrub_markdown）
...
- あなたは歌詞・母音・発音を聞き取れていない（音声認識はしていない）。歌詞や母音を推測・断定せず、場所は必ず「何秒」「どの音名」で指す。ユーザーが歌詞を教えてくれた時だけ、その歌詞に触れてよい。
- **生の数値を文面に出さない（docs/42）。** 数値は判断の根拠として裏で使い、口に出す時は体感の言葉へ翻訳する。文面に書いてよい単位は**秒数・半音・音名だけ**で、cents・dB・％・Hz・点数は書かない。例: ジッター7cents→「声の揺れは落ち着いています」／強弱16dB→「強弱の起伏がしっかり出ています」／20cents低い→「少し低めです」。根拠の実測が無いのに体感語で褒めるのは捏造なので不可。
...
# docs/42 来歴（この台本本文が実装するルール）:
#   やること: [FB-042-04-4]（機構→原因→処方）[FB-042-04-6]（改善提案は1点）
#     [FB-042-04-7]（練習は参考知識を土台に・でたらめ防止）[FB-042-03-1]（発声優先の指針）
#     [FB-042-04-2]（原曲比較の核心差分＝高音の声区の明示）
#   接地: [FB-042-05-1]（証拠に無い数値を作らない）[FB-042-04-1]（原曲比較の事実を必ず根拠に）
#   出力フォーマット: [FB-042-08-1]（録音1本で受付・深掘りは問いかけ経由）[FB-042-08-2]（L0の順序）
#     [FB-042-08-3]（初回に処方・再提出要求を出さない）[FB-042-08-4]（オファーは系統に合わせる）
#     [FB-042-08-5]（1ターン1問）[FB-042-08-7]（練習を求められたらL2へスキップ）
#     [FB-042-08-8]（安全例外は最優先）[FB-042-04-5]（良い点＋改善点を具体的に）
...
def build_session_context(state: dict) -> str:
    ...
    docs/42 来歴: [FB-042-04-1]（原曲比較の実測を根拠として供給）
    [FB-042-04-3]（声区は推定＋信頼度つきで供給し、断定させない）
    [FB-042-05-2]（録音の長さを添えて超過秒数を封じる）
    [FB-042-06-5]（数値直後に体感翻訳のリマインドを置く）
    """
    lines: list[str] = []
    ...
    # 数値を見た直後に翻訳を思い出させる（docs/42「比較は体感の言葉で語る」2026-08-08 運用者FB）。
    # プロンプト冒頭の原則だけでは、事実の直後に並ぶ生の数値をそのまま引用する事故が起きた
    # （実測: 現行Gemini 11件/72会話・GPT 6件/72会話）。数値のすぐ隣にも置いて二重に効かせる。
    lines.append(
        "※上の数値は判断の根拠。文面には出さず体感の言葉に翻訳する"
        "（書いてよい単位は秒数・半音・音名だけ。cents・dB・％・Hz・点数は書かない）"
    )
    return "現在のレッスン状況:\n" + "\n".join(lines)
...
def _finalize_tool_reply(last_text: Optional[str], acc: dict) -> Optional[str]:
    ...
    docs/42 来歴: [FB-042-02-5]（動画URLは会話文末尾に1行）[FB-042-05-6]（曲名は候補提示＋確認）
    """
...
def scrub_card_promise(text: Optional[str]) -> Optional[str]:
    ...
    docs/42 来歴: [FB-042-02-3]（カードを出す約束をしない。検査は checkers.py P2）
...
def scrub_markdown(text: Optional[str]) -> Optional[str]:
    ...
    docs/42 来歴: [FB-042-02-2]（Markdown禁止のコード側ガード。検査は checkers.py P1）
    """
    if not text:
        return text
