...
const FEATURES: Array<{ icon: IconName; title: string; desc: string }> = [
  { icon: "mic", title: "録って送るだけ", desc: "ブラウザでその場録音、または音源をアップロード。" },
  { icon: "bar-chart", title: "声を見える化", desc: "音程・リズム・表現を解析して、やさしく解説。" },
  { icon: "target", title: "今日の課題と基礎練", desc: "あなたに合った練習メニューと参考動画を提案。" },
  { icon: "chart", title: "上達を実感", desc: "録り直すたびに、良くなった点をちゃんと褒めます。" },
];
    ...
    <div className="bg-studio min-h-[100dvh] w-full">
      <div className="mx-auto grid min-h-[100dvh] max-w-6xl grid-cols-1 lg:grid-cols-2">
        ...
        <section className="relative hidden flex-col justify-between overflow-hidden p-10 lg:flex">
          ...
          <div className="absolute -right-10 bottom-10 h-72 w-72 rounded-full bg-brand-200/40 blur-3xl" />

          <BrandWordmark size={44} />

          <div className="relative z-10 space-y-6">
            <div className="animate-floaty inline-flex">
              <IconChip icon="mic" size={80} />
            </div>
            <h1 className="text-4xl font-black leading-tight text-slate-900">
              あなたの歌に、
              <br />
              <span className="text-brand-600">専属トレーナー</span>を。
            </h1>
            <p className="font-body max-w-md text-slate-600">
            ...
            <div className="grid max-w-md grid-cols-2 gap-3">
              {FEATURES.map((f) => (
                <div key={f.title} className="glass rounded-2xl p-3 shadow-card">
                  <IconChip icon={f.icon} size={32} />
                  <div className="mt-1 text-sm font-bold text-slate-800">{f.title}</div>
                  <div className="font-body text-xs text-slate-500">{f.desc}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="flex items-center justify-center p-6">
          <div className="w-full max-w-sm">
            ...
            <div className="glass animate-fade-in-up rounded-2xl p-7 shadow-soft">
